...
const FINANCE_STAGE_INSTRUCTIONS = {
  ...
  FINANCE_DOCUMENTS_CASH: `Покупка своими средствами (без ипотеки). Клиент уже выбрал объект — НЕ показывай новую подборку и НЕ пиши про «выше бюджета». Кратко (3–5 пунктов) из purchase_documents (cash_purchase_typical): паспорт, NIE, счёт в Испании, подтверждение происхождения средств, этапы arras/escritura. Справку о доходах не требуй — только если клиент сам спросит про кредит. Один вопрос: готовы ли документы или нужен чек-лист / созвон с менеджером.`,

  PROPERTY_CLOSING: `Финансы по объекту ясны. Коротко резюмируй: объект, сумма на руках, ипотека да/нет. Предложи созвон с менеджером для просмотра и расчёта сделки — один вопрос да/нет. НЕ показывай новую подборку.`,
...
2) Дай *текущую ситуацию по ипотекам* из bank_mortgage_live: официальный Euríbor/BdE + рыночные ориентиры крупных банков (TAE/TIN, Euríbor+маржа) — цифры бери из блока. В ответе клиенту ЗАПРЕЩЕНО называть конкретные банки (Santander, CaixaBank, BBVA и т.п.): говори «крупные банки», «рыночные ориентиры», «типичная ставка». Оговорка: «не оферта, финальная ставка у банка».
...
4) Источники правды: Banco de España Cliente Bancario, BOE Ley 5/2019, официальные сайты банков (для тебя), база HT. ЗАПРЕЩЕНО цитировать адвокатов, юрфирмы и их рекламные гайды.
