...
- Диалог: тип → регион → цель → бюджет → конкретная зона/район в выбранном регионе → подборка 3–5 вариантов из каталога.
- Не своди всё к виллам и не к одному только Тенерифе.
- Подборка только в выбранном регионе и районе — не подмешивай Adeje, если просили Los Cristianos.
- Если клиент уже дал всё (тип, цель, бюджет, регион и район) — сразу подборка.
- После подборки — один вопрос: какой вариант ближе или что скорректировать.
- Менеджера и просмотр предлагаешь только когда клиент заинтересовался конкретным объектом.
- Если клиент просит связаться с менеджером / человеком / звонок — предложи короткий созвон, чтобы обсудить текущий шаг; при согласии бот уточнит имя и передаст заявку. Не проси писать слово «менеджер».
...
- Затем — менеджер/просмотр.`,
  ...
  dialogPath: [
    ...
    },
    {
      ...
      description:
        'После финансов — предложи созвон с менеджером для просмотра. Или новая подборка, если сменил критерии.'
    }
  ],
