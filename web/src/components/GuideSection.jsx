...
const STEPS = [
  ...
  {
    n: 4,
    text: 'В разделе «БД» смотрите клиентов и историю чатов — они сохраняются между редеплоями.'
  }
];
...
const SECTIONS_GUIDE = [
  ...
  },
  {
    ...
    Icon: IconCatalog,
    what: 'Просмотр уникальных объектов с housetenerife.eu (~600–700 объявлений). На сайте одна квартира может иметь отдельные страницы на ru/es/en — в каталоге бота это один объект, не «1300».',
    edit: [
    ...
    tip: 'Обновление: npm run sync-db (полная) или npm run sync-missing (только пробелы). После синка перезапустите бота.'
  },
  {
    id: 'database',
    title: 'База данных',
    Icon: IconDatabase,
    what: 'Клиенты WhatsApp и история переписки в SQLite: имя, телефон, страна по номеру, языки общения, сообщения с ботом.',
    edit: [
      'Искать пользователей по имени, телефону, стране или языку',
      'Открывать карточку клиента и смотреть историю чата',
      'Проверять статистику: число пользователей, сообщений, активных за неделю'
    ],
    effect:
      'Данные переживают редеплой (файл bot.db на Volume). Бот восстанавливает историю диалога из БД после перезапуска.',
    tip: 'На Railway нужен Volume и SESSION_PATH=/data/... — тогда bot.db тоже лежит в /data.'
  },
  {
    id: 'assistant',
